...
export async function action({request}: ActionFunctionArgs) {
    ...
    const changes = await addUserToRoom(result.data.roomId, result.data.userId)

    if (!changes) {
        return data(null)
    }

    const session = await addToastMessages(request, [new Toast('Added user successfully!', true)])
